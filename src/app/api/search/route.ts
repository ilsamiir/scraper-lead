...
function filterByProvince(results: any[], targetProvince: string): any[] {
...
function deduplicateResults(results: any[]): any[] {
    ...
    return results.filter(r => {
        const key = `${(r.title || '').toLowerCase().trim()}|${(r.address || '').toLowerCase().trim()}`;
...
function mapPlaceToLead(place: any, keyword: string, location: string) {
    let city = null;
    let province = null;
